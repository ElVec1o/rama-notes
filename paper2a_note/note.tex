...
leaves to $k$ hubs instead of two leaves the divisor untouched while raising the connectivity, and
gives a $3$-connected counterexample on $23$ vertices, so the $3$-connected form fails as well. Both
hypotheses constrain the ambient graph; the engine belongs to the divisor, and that is what a repair
must constrain. Everything asserted is machine-checked in Lean~4/Mathlib unless labelled otherwise.
...
So neither hypothesis survives, and they fail for one reason rather than two. Minimum degree and
connectivity are both constraints on the ambient graph, and the engine is a property of the divisor:
a star's leaves have degree one in $H$ whatever their degree in $G$, and enlarging the separator
leaves $A$ untouched while raising $\kappa$. Any hypothesis of the form ``$G$ is locally or globally
well connected'' can be met by attaching the leaves to more hubs, at the cost only of requiring
$p\ge k+1$ branches. What a repair must constrain is the divisor, not its surroundings.
