...
Finer measurement changes the diagnosis once more, and we state the final version. Scanning at step
$0.0005$, and treating non-convergence of the ratio iteration as \emph{unknown} rather than as
membership of the spectrum, the certified gap widths are
\[
  0.0295,\quad 0.1035,\quad 0.1000,\quad 0.0305,\quad 0.0400
\]
for Hall's example and the four above, against a reported $0.040$ in every case. The detector scans
in steps of $0.02$ and scores a non-convergent point as lying in the spectrum, so a gap appears as a
scatter of isolated converged points rather than an interval, and its reported width collapses
toward two scan steps. Between $25$ and $58$ percent of the fine-scan points converge at all, so
even the certified widths are lower bounds.

Two of the four new counterexamples therefore sit in gaps of width about $0.10$, twice the threshold
that discarded them (\texttt{code/gapwidth\_underreport.py}). The failure is not that the phenomenon
lives in narrow gaps. It is that the instrument reports wide gaps as narrow ones. A correct detector
would have found those two at the original threshold, and the useful lesson for anyone searching
this space is to fix the width estimate rather than to lower the cutoff.
...
either setting and no root in any of them is a violation
(\texttt{code/rerun\_lowgap.py}), so those recorded verdicts stand unweakened.
