...
whose leaves have degree one in the divisor however large their degree is in $G$. Joining those
leaves to three hubs instead of two leaves the divisor untouched while raising the connectivity, and
gives a $3$-connected counterexample on $23$ vertices, so the $3$-connected form fails as well. Both
of the hypotheses proposed as repairs constrain the ambient graph, while the engine belongs to the
divisor. We stop there rather than generalising: the construction does not scale past $k=3$ in our
hands, so $4$-connectivity is open. Everything asserted is machine-checked in Lean~4/Mathlib unless
labelled otherwise.
...
engine; Proposition~\ref{prop:c2false} then closes the connectivity route too, by attaching those
leaves to three hubs rather than two. The biregular case, which survives the
...
That reading should not be pushed further than the data carries it, and the data stops short. The
construction does \emph{not} simply scale with $k$: raising the divisibility to $A^{p-k}$ costs
$p\ge k+1$ branches, but it also raises every leaf's degree and fills the gap in. Of the cases run,
$k=3$, $m=3$ violates at $p=5$ and $p=6$ but not at $p=4$, where $\sqrt3$ sits in a band; and both
$k=4$ attempts fail, $(k,m,p)=(4,3,5)$ on $24$ vertices and $(4,4,5)$ on $29$ vertices with
$\kappa=4$, the root lying in a band in each. So $4$-connectivity is untouched by this construction,
and we do not claim it fails. What is established is that the two hypotheses actually proposed as
repairs, minimum degree three and $3$-connectivity, are both false, and that raising an ambient
hypothesis is not by itself a way to escape a star divisor.
