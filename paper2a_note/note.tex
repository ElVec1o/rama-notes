...
Let $S\subseteq V(G)$ with $|S|=k$, and suppose $G-S$ has $p$ components, each inducing a copy of the
same \emph{tree} $B$, with $\theta$ an eigenvalue of $B$. If $p>k$ then $\theta$ is an eigenvalue of
$T_G$, and in particular $\theta\in\operatorname{spec}(T_G)$.
\end{proposition}

\begin{proof}
Take the union $U$ of the $p$ branch vertex sets. Then $G[U]$ is the disjoint union of $p$ copies of
$B$, hence a forest with $\operatorname{cc}(G[U])=p$, and $\theta$ is an eigenvalue of every
component. Every vertex outside $U$ that is adjacent to $U$ lies in $S$, so
$|\partial_G U|\le k<p$. Thus $U$ is a $\theta$-Aomoto subset and \cite{BGM} applies.
\end{proof}

The hypotheses of Proposition~\ref{prop:obstruction} are exactly those of Lemma~\ref{lem:divis}
together with $p>k$, which is what the lemma needs for the exponent $p-k$ to be positive. So
...
(\texttt{code/aomoto\_obstruction.py}).
