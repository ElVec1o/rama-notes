...
\subsection{A caveat on all the negative evidence}\label{sec:blindspot}
...
(\texttt{code/gapfilter\_blindspot.py}). The detector finds the gap correctly; the threshold throws
it away.

Every negative verdict in this development must therefore be read as \emph{no violation was found in
a gap wider than $0.05$}, which is much weaker than it sounds, and which excludes the one
counterexample known to exist. The counterexamples of the two-cut family occupy widths $0.066$ to
$0.148$, which is presumably where the threshold came from; Hall's is narrower than all of them, and
so is the regime the phenomenon actually inhabits. We record this rather than quietly re-running,
because the published searches were carried out at the old setting and their stated scale conveys a
confidence the instrument did not have.
