...
(\texttt{code/hessian.py}). Since the cross differences span the linearised space, the tangent
directions are not closed under subtraction: the commuting point is a \emph{singular} point of the
variety, its tangent cone is not a linear subspace, and a Hessian on the linearised space is the
wrong object. Local maximality therefore remains open, and what it needs is a second-order theory
on that cone.
...
are tangent, by the explicit rotations above, and they span the kernel, so the cone is a proper
subset of the kernel and is not closed under subtraction.
\end{proof}
