...
identically (\texttt{OrderFour.image\_in\_trace\_zero}). That it fills that hyperplane, rank
exactly $n-1$, is measured rather than proved: over $39$ directions across two families the rank
is $n-1$ except at the cross basis directions, where it drops to $1$ and the obstruction vanishes
outright. And the
...
the order-four solve are $10^{-18}$ on the directions tested (\texttt{code/tangentcone.py}).
Orders two, three and four are therefore all satisfiable on the cone; passing from every finite
order to an actual curve is a further step, and is not taken here.

Whether every direction of that cone is the velocity of a genuine curve is not settled here. It
holds wherever it has been looked at: over the $42$ cross basis directions of the Fano family and
...
caveat. Relaxing idempotency to positive semidefiniteness of rank $b$ destroys it immediately, since
$A_k=(b/p)I_p$ is PSD of the right trace with $\sum_kA_k=aI$ and exceeds the band already at
$p=4$, as recorded in \S\ref{sec:leaves}. That is not evidence against Xu's conjecture, which is about projections; it is the
...
\begin{itemize}
\item A barrier argument that sees only the rank, the trace and $\sum_kP_k=aI$ is reading the
  Marchenko--Pastur law, and $A_k=(b/p)I_p$ satisfies all three while exceeding the tree band
