...
So the statement the route proves is true throughout the gap, including where the sign-alternating
certificate provably cannot hold, and what fails at the edge is the invariant. Measuring
$\min_v|F_v|$ across the gap names the replacement: it equals $\lambda$ exactly, attained at a
leaf, at every fraction tested in two of three families, falling to $0.83\lambda$ and
$0.51\lambda$ only at $(3,6,5)$ near the edge (\texttt{code/minF\_scan.py}). The candidate is
therefore $|F_v|\ge\lambda$, which is sign-free and tight.

It does not yet propagate, and the obstruction is now one level down rather than at the level of
the bounds. From $F_v=\lambda-\sum_u1/F_u$ a modulus bound alone gives nothing, because the
$1/F_u$ can cancel. Where the children share a sign they cannot, and the bound goes through; near
the gap edge the signs mix, which is exactly where $\min_v|F_v|$ falls. \textbf{What a proof of
the inner end needs is control of sign mixing among the children of a single vertex, not a
sharper bound}, and that is the form in which we leave it.
