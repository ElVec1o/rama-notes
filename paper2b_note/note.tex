...
$(3,6,5)$ (\texttt{code/depthind.py}); \cite{Supp2} removes that last restriction by
...
This has to be reconciled with \cite{Supp2}, which rules out cavity recursions. The
...
a technical convenience; it is what \cite{Supp2} forces, and it is the reason the route
...
stronger than $X_e\le0$, which is its $c=1$ case, and by \cite{Supp2} it
...
\item No bound of the form $\operatorname{maxroot}\mu\le F(a,b,p,q,\operatorname{spec}A_1,\dots,
  ...
  it applies equally to the Poisson-binomial variance of \cite{Supp2}, which is a spectral
...
(\cite{Supp2}), so Xu's inequality is a theorem there at every $b$ and, by
\cite{Supp2}, the constant $(\sqrt{a-1}+\sqrt{b-1})^2$ is attained and cannot
...
Everything asserted here is machine-checked in Lean~4/Mathlib unless labelled otherwise; the
sources are \texttt{RamaLean/} in the accompanying repository and carry no \texttt{sorry} and no
custom axioms. The standing blocker is that Mathlib has no matching polynomial and no mixed
characteristic polynomial, so the identities connecting the combinatorial and operator forms are
stated rather than derived. Statements are labelled \texttt{VERIFIED}, \texttt{HEURISTIC} or
\texttt{CONJECTURE} in the sources, with their plain meaning: a \texttt{HEURISTIC} claim is
supported by computation and is not proved. The file-by-file record is in \cite{Supp2}.
