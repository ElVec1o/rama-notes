...
question, which is measurable: is the coordinate locus \emph{extremal}? Deform a coordinate family
by rotating each block independently, $U_e=\exp(tX_e)$ with $X_e$ skew, and restore tightness
exactly by $A_e=a\,S^{-1/2}U_eP_eU_e^{\mathsf T}S^{-1/2}$ with $S=\sum_eU_eP_eU_e^{\mathsf T}$; the
$A_e$ stay positive semidefinite of rank exactly $b$, so the path leaves the coordinate locus and
nothing else. Along it the ratio falls monotonically at every hypergraph tried, and an ascent
started at the coordinate point accepted none of $400$ steps at any of four starting points
(\texttt{code/offcoord.py}). On that evidence the coordinate families are strict local maxima,
and Conjecture~1.4 would follow from the case settled above together with the statement that no
deformation off the coordinate locus increases the greatest root. That is a reduction worth
recording, since it replaces a conjecture about all tight rank-$b$ families with a monotonicity
statement over a theorem. It is not proved here, and the evidence is local: the ascent probed
step sizes from $0.25$ down to $0.034$, and below that scale it is the sweep that carries the
weight, which already loses $2\times10^{-4}$ at $t=0.02$.
