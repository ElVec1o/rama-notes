...
\ref{lem:o3trace} puts $R$ in the hyperplane $\sum_jR_j=0$ for every choice; whether the free
entries sweep that whole hyperplane is measured, not proved. At the Fano family they sweep a space of
dimension $6$ against the hyperplane's $6$, and the linear solve returns residual
$1.8\times10^{-14}$: order three is unobstructed there. The claim carries that measured step, so it
is stated as such rather than as a theorem for every family.
