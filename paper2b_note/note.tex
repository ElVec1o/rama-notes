...
The strictness of the hypothesis is a reason for caution, and the caution is warranted. The
containment is measured only for the biregular families of \S\ref{sec:inherit}. At minimum degree
three in general it is \emph{not} established, and a search over $\delta\ge3$ graphs carrying a
gap has produced a candidate failure --- a $12$-vertex graph with degrees in $[3,10]$ whose path
tree appears to carry twelve eigenvalues inside a gap of $\operatorname{spec}(T)$, while no root
of $\mu_G$ lies there (\texttt{code/pathtree\_delta3.py}). That count is not yet trusted, the
first run having discarded the near-zero-pivot flag that makes an inertia count reliable, and it
is being rechecked. If it survives, the containment is false exactly where the conjecture is true,
and Proposition~\ref{prop:reform} is then a reformulation of the biregular case only and no route
to the general one. We state it because the implication is worth having either way, and because
the honest position is that its hypothesis may be too strong.
