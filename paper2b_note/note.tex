...
is a certificate and not a reading. What it is not is a statement about all commuting families:
this is the Fano family, and the general case is open.

One order beyond the obstruction, nothing further is imposed. The order-three equation reads
$Y_k-P_kY_k-Y_kP_k=D_kX_k+X_kD_k$, and its diagonal entries are
$2(D_k)_{jj}(X_k)_{jj}$ since $X_k$ is diagonal; but $D_k$ is supported on pairs with exactly one
endpoint in $e_k$, so its diagonal vanishes and the right side has no diagonal at all. The
order-three obstruction is therefore identically zero, for every direction and not only for those
on the cone, and $Y$ is left free in its off-diagonal blocks. Whether that freedom absorbs the
order-four condition is what a proof of the next statement would have to settle.
