...
$\det[\gcd(i,j)]=\prod_{k\le n}\varphi(k)$, which is itself machine-checked in the general
form $\det[\sum_{d\mid\gcd(i,j)}f(d)]=\prod_{k\le n}f(k)$ for an arbitrary $f$ over a
commutative ring (\texttt{SmithDeterminant.det\_gcdDivSum}, specialised at $f=\varphi$ by
\texttt{det\_gcd\_eq\_prod\_totient}), via the factorization $M=LDL^\top$
...
$c_1!\mid\perm(M_x)$ (\texttt{PermanentFactorial.factorial\_dvd\_permanent\_of\_ones\_rows},
a free \texttt{Perm(Fin }$c_1$\texttt{)} action, from the reusable engine
...
achiever count is computable in $O(n\log n)$ with no permanent (a divisor sieve). Its arithmetic
entry point is machine-checked: a $\gcd$ of two odds is odd and $(g-1)/2$ is odd exactly when
$g\equiv3\pmod4$, so the parity of $\pi_1$ is the parity of the number of entries $\equiv3\pmod 4$,
a count weighted by the non-principal character $\chi_4$ (\texttt{Paper3Chi4.odd\_half\_iff} and
\texttt{sum\_half\_parity}). It is odd for
...
$v_2(\perm(2e+J))=s-a$ exactly} (machine-checked as
\texttt{GeneralEngine.two\_pow\_dvd\_permanent\_odd\_general}; the case $d=0,1$ recovers
...
\texttt{Paper3CMinus1.gcd\_chi4\_nonempty\_of\_three}); and since $\{v:\mathrm{ord}_v(2)=m\}$ consists of divisors of
$2^m-1$,
\[
