...
$\sum_pg_p=\infty$ by Mertens---giving $c_\infty=\infty$ unconditionally.
Finally, we determine the exact $2$-adic \emph{deficit} $v_2(n!)-v_2(a(n))$ at $n=2^k+1$ through a
...
give $D(2^k+1)=2k-4$ whenever a computable, permanent-free ``achiever parity'' is odd---which occurs
for $11$ of the $20$ values $k\le23$, so the deficit is $\Theta(\log n)$ conditional on that parity's
infinitude, the one open step. Reducing that parity through a chain of verified identities
...
Ryser evaluation and, for $n>27$, a modular Ryser with CRT). It is sequence
...
v_2(n!)\sim n$ (slope $1$; deficit $v_2(n!)-v_2(a(n))\in[-1,6]$ through $n=34$, and $\sim2\log_2 n$ along the peaks $n=2^k+1$, where $D(2^k+1)=2k-4$; see Section~\ref{sec:deficit}), and the stronger
...
at the peaks $n=2^k+1$ gives $v_2(a)=3,5,11,25$ for $k=2,3,4,5$, i.e.
...
period-$6$ recurrence, $\chi_4$-reduction, and a MacWilliams character-sum expansion) each terminate at the same parity barrier.
...
classical residue-distribution formula gives
\[
