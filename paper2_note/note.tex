...
exactly $-2a(b-1)^2$. That is why the barrier bound, the scalar free convolution
...
factorization. What does appear to hold is divisibility by a single copy:
$\mu=\psi_0\boxplus_p\rho$ with $\rho$ real-rooted, which by the
Marcus--Spielman--Srivastava bound yields a lower bound on $\lambda_{\min}$ that
reached the band edge in every one of $111$ tested families with $b\ge3$, both
commuting and not. The same test correctly declines to certify the scalar family
$A_k=(b/p)I$, for which $\rho$ fails to be real-rooted; divisibility by $\psi_0$ is
a property the projection class has and the positive semidefinite relaxation does
not, which is precisely the constraint that the free convolution discards.
