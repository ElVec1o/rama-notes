...
$P=a-1$, $Q=b-1$, the right side is
$u^{-n}\prod$-normalized to $\mu_G(\lambda)$ at
...
per transposition, and the diagonal entry $1+(d_v-1)u^2$ at each fixed point.
The biregular substitution is the identity
$\sum_k(-1)^km_ku^{2k}(1+Pu^2)^{p-k}(1+Qu^2)^{q-k}$ rewritten in $\lambda$, and
$\lambda^2=1/t+(P+Q)+mt$ gives the last display since $c=P+Q$.
\end{proof}

Combining with the Ihara--Bass formula for signed graphs, the left side is
$(1-u^2)^{|E|-|V|}$ times the expected characteristic polynomial of the
non-backtracking operator. Since $1/t+mt=2\sqrt m\cos\varphi$ exactly when
$|t|=1/\sqrt m$, Proposition~\ref{prop:gauge} says:
...
Proposition~\ref{prop:gauge} is elementary given \cite{GG} and the signed
Ihara--Bass formula, and we claim no novelty for it beyond the observation that
the composite is what Conjecture~\ref{conj:gap} is about. The nearest statement
we have found in the literature is that of Watanabe and Fukumizu \cite{WF}, whose
loop-sum polynomial carries the Bass prefactor $(1-\beta)^{|E|-|V|}$ and
specializes to the monomer--dimer partition function, though it is not there
identified as a Bass-type formula.
...
at $0.7392$. In the trivial regime the same plateauing is visible and can be
computed to many digits: for the cubic bipartite circulants with connection set
$\{0,1,3\}$, which have girth $6$ for every $n$, the least root tends to
$99.85186\%$ of the lower edge as $n\to\infty$, and for $\{0,1,2\}$, girth $4$, to
$98.46084\%$; both stop strictly short of $100\%$. This is what one should expect:
