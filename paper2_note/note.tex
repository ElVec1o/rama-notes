...
graphs above. So the inequality is far from tight. None of that is available to the
argument:
...
the tightness hypothesis alone. What is \emph{not} formalized is the bookkeeping that
identifies the coordinate vector \texttt{w} of \texttt{CrossTerm} with the Pl\"ucker
coordinates of $\omega_k'$; each ingredient is proved, in its own coordinates, and the
identification between them is by hand.
...
Proposition~\ref{prop:rigid}. The vertex recursion of Theorem~\ref{thm:vertexrec} and the
