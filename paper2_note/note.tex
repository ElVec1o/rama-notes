...
\eqref{eq:twosided} is the known Ramanujan bound $|x|\le s+t$ for bigraphs; the
lower end is the conjecture. \emph{They are the two ends of one interval}, so
...
the multivariate sense, and the univariate case $b=2$ is the accident that the
specialization is again a matching polynomial of a graph.
...
$R=0$ while $D>0$. The invariant region is $R_A<0<1\le R_B$, and the fixed
points do lie there throughout the gap. A $B$-leaf carries $R_B=1$, exactly the
boundary, and is harmless; an $A$-leaf carries $R_A=y>0$, of the wrong sign.
Whether an $A$-leaf can occur, and what it costs, is the remaining obstruction.
