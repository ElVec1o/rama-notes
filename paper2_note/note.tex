...
The exponent is $-2/3$, and that is not an arbitrary number: $n^{-2/3}$ is the soft-edge scale
of random matrix theory, the rate at which extreme eigenvalues approach a spectral edge. Read
that way the picture changes character. The margin tends to zero, so no size-free bound can
prove the biregular case; but it tends to zero \emph{from above}, so the statement is true and
merely tight; and the right target is a Friedman-type edge theorem, in which the roots of
$\mu_G$ fill out $\operatorname{spec}(T)$ without escaping it, exactly as the eigenvalues of a
random $d$-regular graph fill out $[-2\sqrt{d-1},2\sqrt{d-1}]$ without escaping. This is the
Alon--Boppana analogy of \S\ref{sec:biregblock} with an exponent attached.

The structural consequence is worth stating precisely, because it disqualifies a class of
proofs including our own. A quantity that is strictly positive at every size may still admit no
uniform positive lower bound, and then every argument producing a constant independent of $n$
is insufficient in general. That is \texttt{SoftEdge.no\_uniform\_lower\_bound}, with
\texttt{power\_law\_tendsto\_zero} recording that an inverse power law has exactly this shape.
The Gershgorin constant of Theorem~\ref{thm:margin} is such a constant, so it cannot generalise
beyond the complete bipartite family however it is sharpened.

Two limits on this. Equation~\eqref{eq:softedge} rests on fourteen sizes in one family and six
in another, up to $n=68$; the families $(4,8)$, $(4,12)$ and $(5,10)$ did not yield enough
connected samples to fit. And the identification of $-0.6675$ with $-2/3$ is an inference from
the random matrix analogy, not a derivation. What is not in doubt is the monotone decline and
the quality of the power-law fit.
