...
settles $x$, and how often that happens depends sharply on $b$: $80.2\%$ of $486$ midpoints
...
Over $208$ residue points the criterion fires every time, with worst margin $2.149$ and
median $173$ (\texttt{code/jsplit.py}). It is therefore tight rather than comfortable at the
hardest points, and the sample contains only two points with $|K|\ge2$, so it says almost
nothing about those. What the criterion buys is a target with structure: $J_+/J_-$ is large
because $x$ sits near an edge of the crossing band, the minority measure never exceeding
$0.0674$, and $\Lambda$ is a product of band-distance ratios over the non-crossing bands.
Bounding either one is a self-contained problem.
