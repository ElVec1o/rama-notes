...
\subsection{The cut vertex is the mechanism}\label{sec:cutvertex}
...
\end{equation}
and the root is taken from the last factor by tuning $p$ and $H$. The step that produces
\eqref{eq:branch} is exactly the statement that $v$ separates $G$. Without a separating vertex
the matching polynomial does not factor this way and the construction has nothing to tune.
Identity \eqref{eq:branch} and the fact that a root of the last factor is a root of $\mu_G$
are machine-checked as \texttt{CutVertexMechanism.branch\_factor} and
\texttt{root\_of\_branch\_factor}; the separation hypothesis is
\texttt{Separating}, with \texttt{not\_separating\_iff} recording that its negation is
connectedness of every vertex-deleted subgraph.

\begin{conjecture}[C1]\label{conj:twoconn}
Every $2$-connected finite graph satisfies
$\operatorname{Zeros}(\mu_G)\subseteq\operatorname{spec}(T_G)$.
\end{conjecture}

The statement was fixed before the confirming evidence was produced, and the evidence is of
three kinds. Twenty-six $2$-connected graphs with a cycle skeleton, degrees between two and
five, produce no root in any gap (\texttt{code/cycle\_family.py}). Every counterexample in hand
has cut vertices, fifteen in the $31$-vertex graph and eleven in Hall's. And closing those
graphs up destroys the effect: adding a ring of edges until no cut vertex remains returns the
root to $\operatorname{spec}(T)$ in every case tested, five closures of two different graphs,
with the violation surviving exactly as long as some cut vertex does
(\texttt{code/twoconnected.py}).

\begin{center}
\begin{tabular}{lrrl}
graph & $n$ & cut vertices & verdict\\\hline
triangles, depth $3$ & $31$ & $15$ & root in a gap\\
\quad $+$ ring on the far vertices & $31$ & $0$ & no root in any gap\\
Hall $(5,5)$ & $41$ & $11$ & root in a gap\\
\quad $+$ ring on the leaves & $41$ & $0$ & no root in any gap\\
\end{tabular}
\end{center}

Conjecture~\ref{conj:twoconn} is not implied by anything proved here and is not proved here.
What it has in its favour, beyond the data, is that it names the feature the mechanism
actually uses, which the degree conditions did not.
