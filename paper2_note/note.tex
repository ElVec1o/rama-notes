...
Angel--Friedman--Hoary \cite{AFH} with decay rate below one. We verified his certificate
...
The pendant leaves are not incidental. Removing them, so that every $w_i$ has degree $q$,
destroys the effect: over the same $36$ parameter pairs the root always lies inside
$\operatorname{spec}(T)$, with the diagnostic separated by three orders of magnitude.
Verified in \texttt{code/hall\_family.py}; the test is the scaling of the density of states
...
$\operatorname{Zeros}(\mu_{d,G})\subseteq\operatorname{spec}(T)$ holds. Minimum degree at
least two is the first candidate hypothesis, on the evidence of \S\ref{sec:hallfamily}, and
the biregular Problem~1 of Song, Fan and Miao is untouched.
