...
\]
the sum over all matchings $M$ of $G$. If $G$ is $(a,b)$-biregular bipartite with
$P=a-1$, $Q=b-1$, this equals $\mu_G(\lambda)$ up to the normalization
$\lambda=\sqrt{(1+Pu^2)(1+Qu^2)}/u$, and then, writing $t=u^2$,
\[
...
\begin{proof}
Expand the determinant over permutations. A permutation contributes a factor
$s_e$ for each edge it traverses, and $\mathbb E_s[s_e]=0$, so only permutations
traversing every edge an even number of times survive; as each edge can be used
at most twice, these are exactly the involutions, that is the matchings. An
involution with $k$ transpositions has sign $(-1)^k$ and contributes $u^2s_e^2=u^2$
per transposition, and the diagonal entry at each fixed point. The biregular
substitution is the identity
$\sum_k(-1)^km_ku^{2k}(1+Pu^2)^{p-k}(1+Qu^2)^{q-k}$ rewritten in $\lambda$, and
$\lambda^2=1/t+(P+Q)+mt$ gives the last display since $c=P+Q$.
\end{proof}

Almost none of this is new, and it is worth being exact about what is. The
averaging step is the Godsil--Gutman argument \cite{GG} and holds for an
arbitrary diagonal matrix in place of $I+u^2(D-I)$. The right-hand side, with
those degree-dependent weights, is the monomer--dimer partition function
$\Xi_G(-u^2,\lambda)$ with $\lambda_v=1+(d_v-1)u^2$, which Watanabe and Fukumizu
prove equals their polynomial $\omega_G(u^2)$ \cite[Thm.~8]{WF}; for a
$(q+1)$-regular graph they reduce it to $u^{|V|}\mu_G(1/u+qu)$
\cite[Eq.~(29)]{WF}, which in the regular case goes back to Nagle \cite{Na}. They
also record the resulting band correspondence: for a $(q+1)$-regular graph the
roots lie on the circle of radius $1/q$, by \cite{HL}. What does not appear in
\cite{WF} is any average --- their Theorem 10 relates the \emph{unaveraged}
determinant to $\omega_G$ by a sum over deletions of vertex-disjoint cycles, and
they remark that the relation of that theorem to the graph zeta function is not
understood. The observation here is just that averaging over the sign gauge kills
exactly those cycle terms, so that $\omega_G$ \emph{is} an expected
non-backtracking characteristic polynomial; by the signed Ihara--Bass formula,
$\mathbb E_s\det(I-uW_s)$ equals $(1-u^2)^{|E|-|V|}$ times the left-hand side
above. The biregular substitution, which \cite{WF} does not treat, is what
converts this into a statement about the band.
...
number. This is what one should expect:
