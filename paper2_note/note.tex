...
matching polynomial. The invariance is worth stating carefully, since it holds within the tight
class and not outside it: with $\sum_kA_k=aI$ imposed, families sharing the pairs
$\bigl(e_2(A_k),\operatorname{range}A_k\bigr)$ have the same $\mu(x+a)$ even when the $A_k$ are
strongly anisotropic, verified to $10^{-13}$ at eigenvalue ratios up to $49$
(\texttt{code/rl\_conj.py}); drop the tightness and the recentring by $a$ has no meaning.
