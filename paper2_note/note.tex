...
a closed form at first Betti number one, a conjecture,\\
...
We conjecture that they lie in $\operatorname{spec}(T)$ itself, which is a proper
...
The closed form, the exponential formula, the subdivision bound, the coefficient
identities including the $4$-cycle term, the reflection, the $ab$ bound and the
positivity behind the extremal computation are machine-checked in Lean~4 over
Mathlib, with no axioms beyond Lean's standard three.
\end{abstract}
...
Only the passage from the permutation expansion to homology classes remains informal. The
...
constant across a gap while the root count jumps at any root, so a gap contains none. That
...
Lipschitz constant exact instead of measured: $\partial A/\partial\theta_j$ has a single entry
of modulus one together with its conjugate, so its operator norm is $1$, and Weyl gives
$L=\sqrt b$ outright.

Let $\kappa$ be the number of bands whose range contains $x$. If $\kappa=0$ the localization
settles $x$, and how often that happens depends sharply on $b$: over $888$ gap midpoints of
graphs on up to seven vertices, $80.2\%$ of the $b=2$ points lie outside
$\operatorname{spec}(G^{\mathrm{ab}})$ against a few percent of the $b=3$ points
(\texttt{code/residue\_sweep.py}, \texttt{code/residue\_b3.py}). More torus directions fill
the gaps in. Over $3433$ residue points the minority-parity region has measure at most
$0.0674$ and
...
Two caveats keep this in proportion. First, $\kappa=1$ holds at about $97\%$ of residue
points and $\kappa\ge2$ at the rest, so Proposition~\ref{prop:wmean} covers most of the
residue and not all of it. Second, the statement it reduces to is a conjecture and is
labelled as one; what can be said is that a deliberate attempt to break it failed. Over $708$
points with $\kappa=1$ the weighted mean lies in $\operatorname{spec}(T)$ every time and is
never closer than $0.183$ to $x$ (\texttt{code/wmean\_test.py}), and a single coincidence
there would have refuted Conjecture~\ref{conj:gap} outright.
...
Propositions~\ref{prop:moments} and~\ref{prop:transfer} together bound the distance from
