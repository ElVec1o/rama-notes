...
What the certificate does not yet do is close on every path tree. The requirement is
$k>\lambda B$ at each right-type vertex, which near the gap edge tends to
$(q-1)-\sqrt{(d-1)(q-1)}$; measured on real path trees it holds in most cases but not all, and
for a $(3,6)$-biregular graph on $15$ vertices at $\lambda=0.99g$ it fails at $0.21\%$ of
right-type vertices. The defect is precise: such a vertex is in fact safe, because one whose
children are mostly blocked has those children's ratios sitting well below $B$, but a
\emph{uniform} interval cannot express that correlation. Closing the route therefore requires
either an interval depending on the number of children, or an argument that few children force
small child ratios. That is a combinatorial statement about self-avoiding walks rather than a
spectral one, which is the point of the route; it is open.
