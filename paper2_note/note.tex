...
6.3, and contains Problem~1 of Song, Fan and Miao --- its biregular case at $d=1$
--- extending it to all graphs and all $d$.
...
ratio $4a\,C_2/C_3$ decreases steadily in $m$ --- over cubic graphs it runs $3.50$ at
$m=8$, $2.00$ at $m=10$ (Petersen), $1.36$ at $m=12$ (Franklin), $1.03$ at $m=14$
(Heawood) --- and crosses below $1$ at $m=16$, where the M\"obius--Kantor graph gives
$0.826$, and $0.687$ for the Pappus graph at $m=18$. So $C_3\le4aC_2$ is false in
general.

What the same computation shows is that the truncation is what fails, not the conclusion.
Writing $\Sigma_R$ for the partial sum of \eqref{eq:Chat} over $2\le r\le R$, the
sequence oscillates and settles quickly:
\[
\begin{array}{lcrrrr}
\hline
G & m & \Sigma_2 & \Sigma_3 & \Sigma_4 & \Sigma_5\\
\hline
\text{cube} & 8 & 1.283 & 0.916 & 0.938 & \\
\text{Petersen} & 10 & 1.523 & 0.761 & 0.835 & 0.835\\
\text{Franklin} & 12 & 1.703 & 0.454 & 0.723 & 0.704\\
\text{Heawood} & 14 & 1.973 & 0.065 & 0.668 & \\
\text{M\"obius--Kantor} & 16 & 2.141 & -0.452 & 0.669 & \\
\text{Pappus} & 18 & 2.163 & -0.984 & & \\
\hline
\end{array}
\]
$\Sigma_3$ goes negative from $m=16$, $\Sigma_4$ recovers to about $0.7$, and $\Sigma_5$
barely moves. The values are for one direction per graph, fixed by the seed in
\texttt{code/hl\_Cr.py}, which also regenerates Theorem~\ref{thm:Cr} against the
definition of $C_r$. So \eqref{eq:Chat} is in practice a three-term statement,
$C_2-C_3/(4a)+C_4/(16a^2)\ge0$, and any proof must carry $C_4$; two terms provably will
not do. By Theorem~\ref{thm:Cr} all three are differences of a Schur square and the
square of a contraction, with only the contraction obstructing.
...
\texttt{weak\_beats\_ab}). \texttt{RamaLean/CrossTerm.lean} carries Theorems~\ref{thm:C2} and~\ref{thm:Cr}.
...
only if} $x\ge2\sqrt a$; and \texttt{double\_root\_at\_threshold} is the structural
form of the same statement, the factorization
$\rho^2-2\sqrt a\,\rho+a=(\rho-\sqrt a)^2$ that collapses the interval of admissible
thresholds to a point. The vertex recursion of Theorem~\ref{thm:vertexrec} and the
