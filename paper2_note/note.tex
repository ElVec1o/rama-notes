...
\texttt{alt\_count\_last} transferring it from the first vertex to the last. What is not
formalized is the recursion over the path tree, which is the standard well-founded induction on
a finite tree.
...
Solving numerically, the condition holds throughout the gap for $(d,q,r)=(4,8,4)$, $(4,12,4)$
and $(5,10,4)$, and on part of it elsewhere: to $61\%$ of $g$ for $(3,6,4)$, $78\%$ for
$(3,9,4)$, $83\%$ for $(3,12,4)$ and $29\%$ for $(3,6,5)$ (\texttt{code/coupled.py}). This is
the first proof of the inner end of \eqref{eq:twosided} for a nontrivial class. The invariant is
measured to hold outside that range too, so \eqref{eq:closure} is lossy there; closing the rest
needs a sharper combinatorial input than Lemma~\ref{lem:blocking}.
