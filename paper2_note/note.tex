...
\texttt{reduction}); the Godsil--Gutman expansion over cycle types is by hand. The
localization is useful in the negative direction too: any counterexample must live in
...
The second barrier is that the abelian cover cannot see into the region that matters. Every
argument above that runs on the torus family needs $x$ outside
$\operatorname{spec}(G^{\mathrm{ab}})$, and computation shows
$\operatorname{spec}(G^{\mathrm{ab}})$ has almost no internal gaps: on $K_4$, two triangles
joined by an edge, the theta graph and $K_4$ with pendants, the only ones are above and below
the whole spectrum, plus $x=0$ in the last case. The abelian spectrum fills in precisely
where the universal cover still has gaps, which is why every route through the torus family
has been vacuous rather than merely difficult.
...
its natural route is precisely what Proposition~\ref{prop:nohom} rules out. That is now the
only missing piece: with the upper half unconditional, a lower bound on
$I_{\mathrm{right}}$ that beats $m^{3/2}$ would settle feedback vertex number two.
