...
The first is a reflection. For $G=S(H)$ with $H$ $D$-regular the subdivision
identity gives $f_G(y)=\mu_H(y-D)$, and $\mu_H(-z)=(-1)^{|V(H)|}\mu_H(z)$, so
...
\eqref{eq:reflect} fails for $b\ge3$: for $K_{3,4}$ and $K_{3,5}$ one checks
directly that $f_G(2c-y)\ne\pm f_G(y)$. So for $b\ge3$ the lower bound is not a
repackaging of the upper bound but carries information the upper bound does not.
...
the gap collapses to zero. No induction by vertex deletion can therefore prove
the lower bound, which rules out the class of arguments that yields the upper
one.
