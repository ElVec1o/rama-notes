...
term; for $m\le7$ the condition is exactly $4a\,C_2\ge C_3$, which held in every family
tested with a factor of $12.2$ to spare at $m=6$, falling to $3.6$ at $m=8$ and $1.94$ at
$m=10$. The range $m\le5$ does not improve on the two-moment bounds above, so
Theorem~\ref{thm:C2} does not by itself enlarge the unconditional range; what it supplies
is the first structural reason for the sign, and a proof that the obstruction to
\eqref{eq:Chat} lies entirely in the odd coefficients $C_3,C_5,\dots$.
...
\texttt{weak\_beats\_ab}). \texttt{RamaLean/CrossTerm.lean} carries Theorem~\ref{thm:C2}. Its two geometric
...
\texttt{crossTerm\_eq\_sq} proves that they force $C_2$ to equal
