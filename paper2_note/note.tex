...
That observation is real but it does not mean what it appears to mean, and in particular
minimum degree at least two does \emph{not} repair Conjecture~\ref{conj:gap}.
...
the root is the algebraic number $2.85290\ldots$ with minimal polynomial
$x^8-26x^6+187x^4-350x^2+91$, and the cavity fixed point at that value is real, with all
$280$ directed-edge ratio equations satisfied to $3\cdot10^{-15}$, all ratios nonzero, and
decay rate $\rho=0.96669$ carrying a Collatz--Wielandt certificate. The $280$ directed edges
fall into $11$ orbits, so an exact certificate in the style of \cite{HallCE} is within reach;
until it is written the statement rests on a numerical ratio system rather than an exact one,
and we label it accordingly. Verified in \texttt{code/mindeg2.py}.
...
$\operatorname{Zeros}(\mu_{d,G})\subseteq\operatorname{spec}(T)$ holds. By
Proposition~\ref{prop:mindeg2} minimum degree at least two is not the answer. What every
counterexample so far has in common is a wide spread of degrees, from two up to nine in the
smallest one, so a regularity hypothesis rather than a connectivity one is the natural place
to look next; the biregular Problem~1 of Song, Fan and Miao is untouched, as is the
first Betti number one case, where the conjecture is a theorem.
