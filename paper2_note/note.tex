...
What remains is exactly that hypothesis:
\begin{conjecture}[A10$'$]\label{conj:childbound}
Every child ratio at a right-type path-tree vertex with $k$ children is below $k/\lambda$.
\end{conjecture}
It concerns how blocking propagates from a vertex to its children along a self-avoiding walk, so
it is combinatorial rather than spectral, which is the point of the route; it is open.
