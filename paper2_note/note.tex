...
\part{The conjecture, its refutation, and the threshold}
...
\S\ref{sec:further} at its smallest, and it is why Heilmann--Lieb, Godsil's path tree and the
...
$(3,6,5)$ (\texttt{code/depthind.py}). This is the first proof of the inner end of
...
Two limits are known. For $(3,6,5)$ the certificate is saturated at the point it reaches, both
bounds being attained with ratio $1.0000$, so no choice of constants does better
(\texttt{saturation\_blocks\_improvement}); the only other counting fact available, the
parent-to-child drop, extends the range by nothing, since $q-r=1$ there makes it weaker than the
depth bound (\texttt{code/combined.py}). And the sign separation is a consequence of
bipartiteness: on wheels no natural class determines the sign of a path-tree ratio, endpoint
degree, depth parity and child count each being mixed (\texttt{code/d3ratio.py}), so the
certificate does not transfer to Conjecture~\ref{conj:mindeg3} in general.

\part{The biregular case by other routes}
...
\S\ref{sec:ratio} and \S\ref{sec:hallce} each cite one result proved here.
...
What the problem is, at $b=2$, can be said exactly. The operator
$\operatorname{Adj}(A):=\sum_k e_2(A_k)P_{\operatorname{range}A_k}$ and the coefficients
$M_r=\sum_{|T|=r}\|\omega_T\|^2$ depend on the family only through the pairs
$\bigl(e_2(A_k),\operatorname{range}A_k\bigr)$. So the class they define is the class of
weighted $2$-plane families
$\{(c_k,V_k):c_k\ge0,\ \sum_kc_kP_{V_k}\preceq aI\}$, and $F_A$ is the matching
polynomial of such a family; for coordinate planes it is the ordinary weighted
matching polynomial.

That invariance belongs to the wedge form and not to $\mu[\,\cdot\,]$: two families sharing the
pairs $\bigl(e_2(A_k),\operatorname{range}A_k\bigr)$ but with anisotropic $A_k$ have different
mixed characteristic polynomials, the roots differing by order unity rather than by order the
tolerance (\texttt{code/rl\_conj.py}). On projections the two agree, $e_2\equiv1$ leaving no
freedom, which is the identity $\nu_G=\mu[P_1,\dots,P_q]$ above and is verified there to $10^{-13}$;
the plane class is therefore the wedge-form generalisation of the matching polynomial, not the
$\mu[\,\cdot\,]$ one, and results about mixed characteristic polynomials do not transfer to it
automatically.
