...
open. Both statements are formalized.
\end{abstract}
...
The graph enters at the next coefficient, and it does so through a single
invariant. Writing $C_4(G)$ for the number of $4$-cycles, computation on pairs of
graphs sharing $(p,a,b)$ gives
\[
[z^{p-4}]\,g \;=\; U(p,a,b)+C_4(G)
\]
with coefficient exactly one: for $(p,a,b)=(6,2,2)$ the values are
$9,10,12$ at $C_4=0,1,3$; for $(6,3,3)$, $-48$ and $-60$ at $C_4=18$ and $6$;
for $(7,3,3)$, $-126$ for the Heawood graph and $-119$ for a $7+7$ circulant, at
$C_4=0$ and $7$. This is the expected place for it to appear, since four
pairwise disjoint edges are first constrained by a $4$-cycle, and it says that
the conjecture is insensitive to the graph until girth enters. We have not
proved the formula for $[z^{p-4}]g$, only verified it.
