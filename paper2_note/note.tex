...
The value $-2/3$ is right for $q=2d$ and wrong elsewhere, and three checks say the dependence
is real rather than an artefact. It is not a matter of which size variable is used: at fixed
...
$\pm0.016$ against $\pm0.018$ for the minimum. Extending to $q/d=5,6$ the exponent flattens near
$-0.62$ rather than continuing to drift, so the picture is two regimes: $-2/3$ at $q=2d$, and
about $-0.62$ for $q\ge3d$ (\texttt{code/softedge3.py}). The constant behaves the same way: with the exponent
...
We cannot resolve the tension at the sizes reached, and an attempt to do so failed
informatively. Measuring $\beta$ directly requires resolving the density within a window narrow
compared with the band; but each graph contributes only $r\approx15$ positive roots, so such a
window holds about one root per graph, which is the margin itself. The estimate is circular: it
returns $\beta\approx0.16$ where $\tfrac12$ is derivable, and collapses to negative values as
the window narrows (\texttt{code/edgedensity.py}). The likeliest reading is that the $q\ge3d$
families are pre-asymptotic and the true exponent is the universal $-2/3$, but the local-slope
test of \S\ref{sec:softedge} does not show them converging over the range available, so we
leave the discrepancy open and flag it as the weakest point in this section.
