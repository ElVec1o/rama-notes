...
$I_{\mathrm{wrong}}\le m_{\mathrm{wrong}}\,ML\operatorname{diam}$
(\texttt{det\_le\_on\_shell}, \texttt{quadratic\_of\_shell}). The square then requires
$\operatorname{diam}\lesssim m$, which is not automatic in $b$ dimensions and holds for a
shell of width $w$ around a codimension-one locus of bounded area; establishing that is what
remains of the upper half. The lower half, a bound $I_{\mathrm{right}}\ge c>0$, is open, and
