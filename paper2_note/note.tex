...
least be a positive integer. We have nothing on the meaning: that evaluation point is the
edge of the Heilmann--Lieb support, whereas everything above happens strictly outside it,
and the mechanism of Proposition~\ref{prop:transfer} says nothing there. The observation
itself is elementary and holds. With $|V(G)|$ even every exponent in $\mu_G$ is even, so
$\mu_G$ is a polynomial in $x^2$ with integer coefficients and $x^2=4(d-1)$ is an integer;
positivity holds because a monic polynomial is positive above its largest root, and
Heilmann--Lieb is strict on a finite graph. Both halves are machine-checked in
\texttt{RamaLean/EvenEval.lean}, together with the observation that the parity hypothesis is
needed: for $|V(G)|$ odd the value is $2\sqrt{d-1}$ times an integer. The values themselves
(\texttt{code/csikvari\_eval.py}, exact) show no pattern we can see: at $d=3$ they run
$19$, $74$, $76$, $297$, $1162$, $18408$, $73825$, $295979$ for $K_4$, $K_{3,3}$, the prism,
the cube, Petersen, Heawood, M\"obius--Kantor and Pappus.
\end{remark}
