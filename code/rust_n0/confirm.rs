...
fn main() {
    let nt = thread::available_parallelism().map(|x| x.get()).unwrap_or(4);
    eprintln!("using {} threads", nt);
    let v33 = v2(n0_fast(33, nt, false));
    println!("n= 33  v2(N0)={}  (sanity, must be 25: {})", v33, if v33==25 {"OK"} else {"FAIL"});
    for &nn in &[63usize, 67] {
        let st = Instant::now();
        let vn = v2(n0_fast(nn, nt, true)) as i64;
        let d = (nn as i64 - s2(nn as u64) as i64) - vn;
        println!("n= {}  v2(N0)={}  v2(n!)={}  D_N0={}   ({:?})", nn, vn, nn as i64 - s2(nn as u64) as i64, d, st.elapsed());
    }
}
