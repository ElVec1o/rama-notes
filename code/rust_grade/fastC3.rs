...
fn main() {
    ...
    // same-prime pairs: only primes p with p^3<=n have >=2 powers ≡3(4) that are <=n.
    // regenerate them and locate power indices by binary search in ppv.
    {
        let idx_of = |val: u64| -> usize { ppv.binary_search(&val).unwrap() };
        let mut p: u64 = 3;
        while p * p <= n {
            // is p prime? check via odd sieve (p is odd here since starting 3 step 2)
            let pi = ((p - 1) / 2) as usize;
            let pprime = !is_set(&comp, pi);
            if pprime && p % 4 == 3 {
                // powers p, p^3, p^5, ... (≡3 mod4) <= n
                let mut g: Vec<usize> = Vec::new();
                let mut qa = p;
                while qa <= n { g.push(idx_of(qa)); if qa > n / p { break; } qa *= p; }
                if g.len() >= 2 {
                ...
                for &b in &g {
                    ...
                    for ia in 0..g.len() {
                        for ib in (ia + 1)..g.len() {
                            ...
                            let mat = (r[a] & r[b]) ^ rl;
                            if act == 1 { s0b ^= mat; }
                            // FIX: remove spurious binom contribution for this same-prime pair
                            s0b ^= nod[a] & r[a] & nod[b] & r[b];
                        }
                    }
                }
            }
    ...
    drop(comp); // free the ~n/16-byte sieve before allocating the aa/ab bit-arrays
