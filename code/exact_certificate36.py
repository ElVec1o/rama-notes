...
WHY THE LAST STEP TO AN EXACT CERTIFICATE IS NOT TAKEN, and a warning attached to the attempt.

Hall's ratios lie in Q(sqrt5, sqrt41), a field of degree four, which is what lets his certificate be
exact. It is natural to expect the same here, since Aut(G) has order 5040 and the twelve orbits look
highly structured. It is false.

Two of the twelve ratios are in Q(theta) and have clean closed forms, both forced by the shape of
the graph rather than by any symmetry:

    r_3  = theta (theta^4 - 12 theta^2 + 25)/7 = 1/theta,   the direction into the pendant vertex,
    r_10 = theta (-theta^4 + 11 theta^2 - 14)/7.

The other ten are not in Q(theta), and are not of low degree over Q at all: at 200 digits, PSLQ
finds no polynomial relation of degree at most 30 with coefficients below 1e20 for any of
r_0, r_1, r_5, r_7. So the field generated by the ratios is large, and writing them down explicitly
is not a practical route to an exact certificate. A large automorphism group does not imply a small
ratio field, which is worth knowing before anyone repeats the attempt.

THE WARNING. Run at 120 digits with a tolerance of 1e-70 and a coefficient bound of 1e14, PSLQ
returned confident-looking expressions in Q(theta) for ALL TWELVE ratios, with twelve-digit
denominators. Ten of them are spurious. They were caught only by substituting back and reducing
exactly in Q[x]/(f), where ten of the twelve equations failed by large margins rather than
marginally. Integer-relation output at a fixed tolerance is a hypothesis, not a result, and the
